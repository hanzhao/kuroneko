...
object MovieLensIncrementalGradientDescentInDuration {
  ...
  def main(args: Array[String]): Unit = {
    ...
    1.until(50).foreach { i =>
      ...
      val model = new MovieLensIncrementalGradientDescentInDurationModel(args(5).toInt)
...
class MovieLensIncrementalGradientDescentInDurationModel(val rank: Int,
  ...
  def train(rating: MovieLensRating, T: Int, gammaU: Float, gammaI: Float, lambda: Float): Unit = {
    0.until(T).foreach { _ =>
      val uf = userFeatures.getOrElse(rating.user, MovieLensIncrementalGradientDescentInDurationUtil.randomVector(rank))
      val pf = productFeatures.getOrElse(rating.product, MovieLensIncrementalGradientDescentInDurationUtil.randomVector(rank))
...
object MovieLensIncrementalGradientDescentInDurationUtil extends Serializable {
